...
main()
{
    ...
    for (int i = 0; i < productIndex; i++)          // Reset the items when the program starts
    {
...
void start_menu(string user_Names_data[], string paswords_data[], string roles_data[], int &userName_index, string products[], int productPrice[], int items_bought_by_user[], int total_bought_items[], int &taxRate, int &productIndex, int &totalRevenue, string &admin_code, string &theme_color, string &runing_username, string themes_data[], string complains[], int &complains_index, string review[], int ratings[], int &review_index, int &theme_index)
{
    ...
    while (true)
    {
        ...
        else if (key == 13)         // Enter Key
        {
            if (current_opt == 0)
            {
                sign_in(user_Names_data, paswords_data, roles_data, userName_index, products, productPrice, items_bought_by_user, total_bought_items, taxRate, productIndex, totalRevenue, admin_code, theme_color, runing_username, themes_data, complains, complains_index, review, ratings, review_index, theme_index);
            }
            else if (current_opt == 1)
            {
                sign_Up_in(user_Names_data, paswords_data, roles_data, userName_index, products, productPrice, items_bought_by_user, total_bought_items, taxRate, productIndex, totalRevenue, admin_code, theme_color, runing_username, themes_data, complains, complains_index, review, ratings, review_index, theme_index);
            }
...
string signIn(string userName, string user_Names_data[], string password, string paswords_data[], string roles_data[], int &userName_index, string &admin_code)
{
    ...
    string result;
    bool isAuser;
    isAuser = doesUserNameExists(userName, user_Names_data, userName_index);        // Check if username already exists
    if (isAuser)
    {
        index = userNameIndex(userName, user_Names_data, userName_index);           // Finds index of entered username
...
// Sign Up Function
void sign_Up_in(string user_Names_data[], string paswords_data[], string roles_data[], int &userName_index, string products[], int productPrice[], int items_bought_by_user[], int total_bought_items[], int &taxRate, int &productIndex, int &totalRevenue, string &admin_code, string &theme_color, string &runing_usernme, string themes_data[], string complains[], int &complains_index, string review[], int ratings[], int &review_index, int &theme_index)
{
    if(getch() == 27){
        return;
    }
    ...
    if (isValid_UserName(userName))     // Username Validation
    {
        ...
        if(password_vald(password)){            // Password Validation
            ...
            else if (signUp_op == "manager")
            {
                cin.clear();
                cin.sync();
                cout <<"\t\t\t\t\t\t\t"<< "Enter admin code: ";     // Special Code to Sign Up as a Manager
                ...
                else
                {
                    ...
                    if (errorChoice == 0)
                    {
                        sign_Up_in(user_Names_data, paswords_data, roles_data, userName_index, products, productPrice, items_bought_by_user, total_bought_items, taxRate, productIndex, totalRevenue, admin_code, theme_color, runing_usernme, themes_data, complains, complains_index, review, ratings, review_index, theme_index);
                    }
                    else if (errorChoice == 1)
                    {
                    start_menu(user_Names_data, paswords_data, roles_data, userName_index, products, productPrice, items_bought_by_user, total_bought_items, taxRate, productIndex, totalRevenue, admin_code, theme_color, runing_usernme, themes_data, complains, complains_index, review, ratings, review_index, theme_index);
                    }
                }
            }
...
void tryAgain_Goback(int &errorChoice,string header){       // To be called in when invalid Input in Sign in & Sign Up
...
bool doesUserNameExists(string userName, string user_Names_data[], int &userName_index)     // User Name Checker
{
...
int userNameIndex(string userName, string user_Names_data[], int &userName_index)       // To obtain username Index
{
    for (int i = 0; i < userName_index; i++)
    {
        if (user_Names_data[i] == userName)
...
void manager(string products[], int ProductPrice[], int items_bought_by_user[], int total_bought_items[], int &productIndex, int &taxRate, int &totalRavenue, string user_Names_data[], string roles_data[], string passwords_data[], int &userName_index, string &admin_code, string &theme_color, string &runing_username, string themes_data[], string complains[], int &complains_index, string review[], int ratings[], int &review_index, int &theme_index)
{
    ...
    while (true)
    {
        ...
        else if (key == 13)
        {
            ...
            {
                editStock(products, ProductPrice, items_bought_by_user, total_bought_items, productIndex);
            }
            ...
            {
                printSalesRecord(products, ProductPrice, total_bought_items, taxRate, productIndex);
            }
...
void showUsers(string user_Names_data[], string roles_data[], int &userName_index)     // Show all users to Manager
{
    system("cls");
    printHeader();
...
void removeUser(string user_Names_data[], string passwords_data[], string roles_data[], int &userName_index)    // Remove User Function
{
    string user;
    int index;
    printHeader();
    ...
    else if (roles_data[index - 1] == "manager")    // Only Users can be deleted, manager can't be
    {
    ...
    {
        user = user_Names_data[index - 1];                                  // Takes the Selected index in "user"
        for (int i = index - 1; i < userName_index - 1; i++)    // Makes the selected index data equal to next index
        {
            user_Names_data[i] = user_Names_data[i + 1];
            passwords_data[i] = passwords_data[i + 1];
            roles_data[i] = roles_data[i + 1];
        }
        user_Names_data[userName_index - 1] = "";           // Places null at the selected index
...
void checkTotalRavenue(int &totalRavenue)               // Displays Total Revenue Generated from sales
{
    system("cls");
    printHeader();
...
void printSalesRecord(string products[], int productPrice[], int total_bought_items[], int &taxRate, int &productIndex)
{
    system("cls");
    ...
    {
        cout <<"\t\t\t\t\t\t\t" << "# " << setw(12) << left << i + 1 << setw(15) << left << products[i] << setw(13) << total_bought_items[i] << "      #\n";
    }
    cout <<"\t\t\t\t\t\t\t" << "#-----------------------------------------------#\n";
    cout <<"\t\t\t\t\t\t\t" << "#################################################\n\n";
...
int calculateTotalSale(int productPrice[], int total_bought_items[], int &taxRate, int &productIndex)
{
...
{
                                    // A sub-Function where manager can Update item rates and tax rate.
...
{
    system("cls");                      // Check listed items in the store
    printHeader();
...
void editStock(string products[], int ProductPrice[], int items_bought_by_user[], int total_bought_items[], int &productIndex)
{
    ...
    // Keys
        ...
        else if (key == 13)
        {
            if (current_opt == 0)
            {
                addProduct(products, ProductPrice, items_bought_by_user, total_bought_items, productIndex);
            }
            else if (current_opt == 1)
            {
                remove_item(products, ProductPrice, items_bought_by_user, total_bought_items, productIndex);
            }
            else if (current_opt == 2)
            {
                break;
...
void addProduct(string products[], int ProductPrice[], int items_bought_by_user[], int total_bought_items[], int &productIndex)
{                           // Function to add item in stock
    ...
    while(true){
        ...
        {
            if(current_opt == 0){
                if (!checkProduct(products, product_name, productIndex))            // Check if it already present in stock
                {
                    products[productIndex] = product_name;          // ADDS item in stock
...
void remove_item(string products[], int productPrice[], int items_bought_by_user[], int total_bought_items[], int &productIndex)
{
...
bool checkProduct(string products[], string product, int &productIndex)     // Check Wether the product present in stock
{
    for (int i = 0; i < productIndex; i++)
    {
...
void change_Admin_Code(string &admin_code)          // Change Special Admin Code
{
    printHeader();
...
// User Menu
void user(string products[], int productPrice[], int items_bought_by_user[], int total_bought_items[], int &productIndex, int &taxRate, int &totalRevenue, string usernames_data[], string &theme_color, string &runing_username, string passwords_data[], int &userName_index, string themes_data[], string complains[], int &complains_index, string review[], int ratings[], int &review_index, int &theme_index)
{
    ...
    while (true)
    {
        ...
        else if (key == 13)     // Enter Key
        {
            ...
            {
                View_Cart(products, productPrice, items_bought_by_user, productIndex, taxRate);
            }
            ...
            {
                fn_paybill(items_bought_by_user, total_bought_items, productPrice, productIndex, taxRate, totalRevenue);
            }
...
{                                               // Buy items from store
    ...
    cout << endl;
    for (int i = 0; i < productIndex; i++)              //  Again Clears All items bougth 
    {
        items_bought_by_user[i] = 0;
    }
    int option, number;
...
void View_Cart(string products[], int productPrice[], int items_bought_by_user[], int &productIndex, int &taxRate)
{
    system("cls");                      // View Cart with bill and tax
    printHeader();
    ...
    for (int i = 0; i < productIndex; i++)
    {
        cout <<"\t\t\t\t\t\t\t"<< "#     " << setw(8) << left << i + 1 << setw(12) << left << products[i] << setw(12) << productPrice[i] << setw(12) << items_bought_by_user[i] << setw(11) << items_bought_by_user[i] * productPrice[i] << "     #\n";
    }
    cout <<"\t\t\t\t\t\t\t"<< "#                                                                 #\n";
...
void fn_paybill(int items_bought_by_user[], int total_bought_items[], int productPrice[], int &productIndex, int &taxRate, int &totalRevenue)
{
    system("cls");                          // Pay bill Function
    printHeader();
    smallMenu("User Menu> Pay Bill");
    int bill, tax, totalBill;
    for (int i = 0; i < productIndex; i++)              // Puts all bought items by user in Total Bougth items
    {
    ...
    totalRevenue += totalBill;              // Adds total bill to revenue
        ...
        // Customer Support Function where user can Take many actions
...
{                                                                       // Complain Function
    ...
    complains[complains_index] = complain_content;              //  Adds complain to database
    complains_index++;
...
{
    printHeader();
    smallMenu("Show Complains");                                // Show Complains to only manager
    cout << endl;
...
{
    printHeader();
    smallMenu("Social Accounts Page");                  // A void page for to displays Social Accounts
    cout << endl;
    ...
    getch();
                    ...
                    //  Both user and manager can use this function
...
{
    printHeader();                      // Both user and manager can see reviews and ratings
...
void change_Password(string runing_username, string passwords_data[], string user_Names_data[], int &userName_Index)
{
    ...
    int index = userNameIndex(runing_username, user_Names_data, userName_Index);        // Gets the index of runing username
    int choice;
    string old_password, newpassword;
    cout << endl;
    cin.clear();
    cin.sync();
    cout <<"\t\t\t\t\t"<< "Enter your Previous Password: ";     // Confirmation of old pasword
    ...
    if (old_password == passwords_data[index])
    {
        ...
        if(newpassword.length()>=6 && password_vald(newpassword)){      // Validates New Password
        ...
        else
        {
            ...
            Sleep(3300);
            change_Password(runing_username, passwords_data, user_Names_data, userName_Index);      // Try Again
        }
    }
    else
    {
...
void changeSystemColor(string &theme_color, string themes_data[], int &userName_index, int &theme_index)
{
    ...
    cout << "\t\t\t\t\t\t\t" << "3 = Aqua        B = Light Aqua" <<endl;                // Displays Manual
    ...
    else                // Validation
    {
        ...
        Sleep(1500);
        changeSystemColor(theme_color, themes_data, userName_index, theme_index);       // Again Redirects 
    }
}
...
bool is_valid_color_code(string color_code)         // Color Code VALIDATION IN CHANGE THEME
{
...
void hideAndVisibleCursor(bool isShow)      // Hide cursor Function
{
...
void saveProducts(string products[], int productPrice[], int items_bought_by_user[], int total_bought_items[], int productIndex) {
    fstream dataFile;
...
void loadProducts(string products[],int Productprices[],int items_bought_by_user[],int total_bought_items[],int &productIndex){
    string data;
    productIndex = 0;
    fstream file;
...
void smallMenu(string submenu)          // Small Menu Navigator
{
...
void SignUp_Header()                // Sign Up Header
{
    cout << R"(
                    ##########################################################################################################
                    ##------------------------------------------------------------------------------------------------------##
...
void SignIn_Header()            // Sign In Header
{
    cout << R"(
                    ###########################################################################################################################
                    ##-----------------------------------------------------------------------------------------------------------------------##
